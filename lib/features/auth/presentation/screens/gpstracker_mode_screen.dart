...
// ─────────────────────────────────────────────────────────────────────────────
// GPS Yayıcı Rejimi — Test üçün ikinci telefonda istifadə edilir.
//
// Necə işləyir:
//  1. İkinci telefon eyni hesabla daxil olur
//  2. Bu ekranı açıb bir heyvan seçir
//  3. "Başlat" düyməsinə basır
//  4. Telefon hər GPS yeniləməsini seçilmiş heyvanın mövqeyi kimi Firestore-a yazır
//  5. Birinci telefondakı TrackingService geofence yoxlar → bildiriş göndərir
// ─────────────────────────────────────────────────────────────────────────────
  ...
  Future<void> _start() async {
    ...
    _gpsSub = Geolocator.getPositionStream(
      locationSettings: const LocationSettings(
        ...
        distanceFilter: 5, // hər 5 metr dəyişiklikdə
      ),
    ).listen(
      (pos) => _onPosition(pos),
      onError: (e) {
        AppLogger.xeta('GPS TRACKER MODE', 'GPS xətası', xetaObyekti: e);
        _snack('GPS xətası: $e', isError: true);
      },
    );
  }

  void _onPosition(Position pos) {
    if (!mounted || _selectedAnimal == null) return;
    ...
    AppLogger.melumat(
        'GPS TRACKER MODE',
        '${_selectedAnimal!.name} → (${pos.latitude.toStringAsFixed(6)}, '
        '${pos.longitude.toStringAsFixed(6)})');
  }

  // Sadə dairə yoxlama (geofencing_service-dən asılı olmadan)
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        const Icon(Iconsax.info_circle, color: Color(0xFF185FA5), size: 18),
        const SizedBox(width: 10),
        const Expanded(
          child: Text(
            'Bu rejim ikinci telefonla GPS testini asanlaşdırır.\n'
            'Heyvan seçin → Başladın → Telefonu gəzdirin.\n'
    ...
    return Container(
      ...
      child: Column(children: [
        ...
        Row(children: [
          ...
          Text(
            isRunning ? '● Yayım aktiv' : '○ Yayım dayandırılıb',
            style: TextStyle(
                fontSize: 12,
                fontWeight: FontWeight.w600,
                color: isRunning
  ...
  Widget build(BuildContext context) {
    return SizedBox(
      ...
      child: ElevatedButton.icon(
        ...
        label: Text(
          isRunning ? 'Dayandır' : 'GPS Yayımını Başlat',
          style: const TextStyle(
