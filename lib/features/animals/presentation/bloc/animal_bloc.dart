...
class AddAnimalEvent extends AnimalEvent {
  ...
  @override List<Object?> get props => [name, type, ownerId];
}
...
class EditAnimalEvent extends AnimalEvent {
  ...
  @override List<Object?> get props => [animalId, name, type];
}

class DeleteAnimalEvent extends AnimalEvent {
  final String animalId;
  const DeleteAnimalEvent(this.animalId);
  @override List<Object?> get props => [animalId];
}
...
class AnimalInitial extends AnimalState { const AnimalInitial(); }
class AnimalLoading extends AnimalState { const AnimalLoading(); }

class AnimalLoaded extends AnimalState {
  final List<AnimalEntity> animals;
  const AnimalLoaded(this.animals);
  @override List<Object?> get props => [animals];
}
...
class AnimalBloc extends Bloc<AnimalEvent, AnimalState> {
  ...
  Future<void> _onWatch(
    ...
    await emit.forEach(
      ...
      onError: (e, _) {
        AppLogger.xeta('ANIMAL BLOC', 'Xəta', xetaObyekti: e);
  ...
  Future<void> _onAdd(
      AddAnimalEvent event, Emitter<AnimalState> emit) async {
    ...
    try {
      ...
      AppLogger.ugur('ANIMAL BLOC', '"${event.name}" uğurla əlavə edildi');
    } catch (e) {
      AppLogger.xeta('ANIMAL BLOC', 'Əlavə etmə xətası', xetaObyekti: e);
      emit(AnimalError(e.toString()));
    }
  }

  Future<void> _onEdit(
      EditAnimalEvent event, Emitter<AnimalState> emit) async {
    ...
    try {
      ...
      AppLogger.ugur('ANIMAL BLOC', '"${event.name}" uğurla yeniləndi');
    } catch (e) {
      AppLogger.xeta('ANIMAL BLOC', 'Redaktə xətası', xetaObyekti: e);
      emit(AnimalError(e.toString()));
    }
  }
  ...
  Future<void> _onDelete(
    ...
    try {
      ...
      AppLogger.xeta('ANIMAL BLOC', 'Silmə xətası', xetaObyekti: e);
      emit(AnimalError(e.toString()));
    }
  }
  ...
  Future<void> _onUpdateLocation(
    ...
    try {
      await _repo.updateLocation(
          event.animalId, event.lat, event.lng, event.speed, event.battery);
    } catch (e) {
      AppLogger.xeta('ANIMAL BLOC', 'Mövqe yeniləmə xətası',
          xetaObyekti: e);
    }
  }
